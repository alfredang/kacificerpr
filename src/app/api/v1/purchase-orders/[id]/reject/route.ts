...
export const POST = withApi({ scope: "approve:po", action: "po.approve" }, async ({ principal, req, onBehalfOf }, params) => {
  ...
  await decidePo(po.id, "reject", { actor: { type: "api_key", id: principal.keyId, label: `API key ${principal.name}${onBehalfOf ? ` for ${onBehalfOf}` : ""}` }, role: principal.user.role, note: body.note, approverId: principal.user.id, via: "api" });
