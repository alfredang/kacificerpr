...
/* The Hermes conversation layer used by both the in-app widget and the
   Telegram bot: store the user's message, run the chat agent, store the reply. */
export async function chatHistory(where: { userId?: string; externalChatId?: string }, limit = 30) {
  const db = getDb();
  const rows = await db
    .select()
    .from(chatMessages)
    .where(where.userId ? eq(chatMessages.userId, where.userId) : and(eq(chatMessages.channel, "telegram"), eq(chatMessages.externalChatId, where.externalChatId ?? "")))
    .orderBy(desc(chatMessages.createdAt))
    .limit(limit);
  return rows.reverse();
}

export async function hermesReply(input: { channel: "widget" | "telegram"; user: User | null; externalChatId?: string; text: string }) {
  const db = getDb();
  ...
  const run = await runAgent("chat", { prompt: input.text }, input.user?.id ?? null);
